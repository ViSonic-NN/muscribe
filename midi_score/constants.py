...
ibiVocab = (
    int(4 / resolution) + 1
)  # vocabulary size for inter-beat-interval: 4s = 4/0.01s + 1, index 0 is ignored during training
...
keySharps2Number = dict(
    [(sharp, keyName2Number[keySharps2Name[sharp]]) for sharp in keySharps2Name.keys()]
)
keyNumber2Sharps = dict(
    [
        (number, keyName2Sharps[keyNumber2Name[number]])
        for number in range(len(keyNumber2Name))
    ]
)
keyVocabSize = (
    len(keySharps2Name) // 2
