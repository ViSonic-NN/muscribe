...
from ..constants import min_bpm, tolerance
from ..midi_read import read_note_sequence
from ..models.beat import RNNJointBeatModel
from ._processor import MIDIProcessor


class RNNJointBeatProcessor(MIDIProcessor):
    def __init__(
        self,
        model_state_dict_path="_model_state_dicts/beat/RNNJointBeatModel.pth",
        **kwargs
    ):
    ...
    def pps(beat_probs, downbeat_probs, onsets, prob_thresh=0.5, penalty=1.0):
        ...
        beats_min = beats[
            np.concatenate([[True], np.abs(np.diff(beats)) > tolerance * 2])
        ]
        beats_max = beats[::-1][
            np.concatenate([[True], np.abs(np.diff(beats[::-1])) > tolerance * 2])
        ][::-1]
        beats = np.mean([beats_min, beats_max], axis=0)
        downbeats_min = downbeats[
            np.concatenate([[True], np.abs(np.diff(downbeats)) > tolerance * 2])
        ]
        downbeats_max = downbeats[::-1][
            np.concatenate([[True], np.abs(np.diff(downbeats[::-1])) > tolerance * 2])
        ][::-1]
        ...
        for i in range(2, len(beats)):
            ...
            for x in range(4):
                ...
                for x_prev in range(4):
                    o1 = np.abs(
                        np.log(ibi / (beats_dp[x_prev][-1] - beats_dp[x_prev][-2]))
                    )
                ...
                beats_dp_new[x] = (
                    beats_dp[x_prev_best]
                    + [beats[i - 1] + ibi * k for k in range(1, x + 1)]
                    + [beats[i]]
                )
