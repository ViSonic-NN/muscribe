...
from ..constants import keyNumber2Name
...
class RNNKeySignatureProcessor(MIDIProcessor):
    def __init__(
        self,
        model_state_dict_path="_model_state_dicts/key_signature/RNNKeySignatureModel.pth",
        **kwargs
    ):
    ...
    def process(self, midi_file, **kwargs):
        ...
        # Post-processing
        key_idx = (
            key_probs[0].topk(1, dim=0)[1].squeeze(0).cpu().detach().numpy()
        )  # (seq_len,)
