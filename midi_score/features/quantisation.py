...
from ..constants import N_per_beat
...
from .beat import RNNJointBeatProcessor
...
class RNNJointQuantisationProcessor(MIDIProcessor):
    ...
    def process(self, midi_file, **kwargs):
        ...
        (
            beat_probs,
            downbeat_probs,
            onset_position_probs,
            note_value_probs,
        ) = self._model(x)

        # Post-processing
        onset_positions_idx = (
            onset_position_probs[0].topk(1, dim=0)[1].squeeze(0)
        )  # (n_notes,)
        ...
        beats, onset_positions, note_values = self.pps(
            onset_positions_idx, note_values_idx, beat_probs, downbeat_probs, onsets
        )

        return beats, onset_positions, note_values

    def pps(
        self, onset_positions_idx, note_values_idx, beat_probs, downbeat_probs, onsets
    ):
        # post-processing
