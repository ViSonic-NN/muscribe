...
from ..constants import tsIndex2Deno, tsIndex2Nume
...
class RNNTimeSignatureProcessor(MIDIProcessor):
    def __init__(
        self,
        model_state_dict_path="_model_state_dicts/time_signature/RNNTimeSignatureModel.pth",
        **kwargs
    ):
    ...
    def process(self, midi_file, **kwargs):
        ...
        # Post-processing
        td_idx = (
            td_probs[0].topk(1, dim=0)[1].squeeze(0).cpu().detach().numpy()
        )  # (seq_len,)
        tn_idx = (
            tn_probs[0].topk(1, dim=0)[1].squeeze(0).cpu().detach().numpy()
        )  # (seq_len,)
    ...
    def pps(self, td_idx, tn_idx, onsets):
        ...
        for i in range(len(td_idx)):
            ts_cur = "{:d}/{:d}".format(
                tsIndex2Nume[tn_idx[i]], tsIndex2Deno[td_idx[i]]
            )
