...
from ..constants import ibiVocab
...
class RNNJointBeatModel(nn.Module):
    def __init__(self, hidden_size=512):
        ...
        self.out_beat = LinearOutput(
            in_features=hidden_size, out_features=1, activation_type="sigmoid"
        )
        self.out_downbeat = LinearOutput(
            in_features=hidden_size, out_features=1, activation_type="sigmoid"
        )
        self.out_tempo = LinearOutput(
            in_features=hidden_size, out_features=ibiVocab, activation_type="softmax"
        )
    ...
    def forward(self, x):
        ...
        x_gru_downbeat = self.gru_downbeat(
            x_gru_beat
        )  # (batch_size, seq_len, hidden_size)
        x_gru_tempo = self.gru_tempo(
            x_gru_downbeat
        )  # (batch_size, seq_len, hidden_size)
