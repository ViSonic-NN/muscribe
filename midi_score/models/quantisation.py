...
from ..constants import nvVocab, omVocab
from .beat import RNNJointBeatModel
...
class RNNJointQuantisationModel(nn.Module):
    def __init__(
        self,
        beat_model_checkpoint="_model_state_dicts/beat/RNNJointBeatModel.pth",
        hidden_size=512,
    ):
        super().__init__()
        ...
        self.out_onset = LinearOutput(
            in_features=hidden_size, out_features=omVocab, activation_type="softmax"
        )
        self.out_value = LinearOutput(
            in_features=hidden_size, out_features=nvVocab, activation_type="softmax"
        )
    ...
    def forward(self, x):
        ...
        x_concat_onset = torch.cat(
            (x_conv_onset, y_beat.unsqueeze(2), y_value), dim=2
        )  # (batch_size, seq_len, hidden_size + 1 + noteValueVocab)
        x_gru_onset = self.gru_onset(
            x_concat_onset
